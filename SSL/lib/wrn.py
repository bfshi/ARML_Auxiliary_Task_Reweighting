...
class WRN(nn.Module):
    ...
    def __init__(self, width, num_classes, transform_fn=None):
        ...
        self.rotate_output = nn.Linear(filters[3], 4)
        self.jigsaw_output = nn.Linear(filters[3], 31)
    ...
    def forward(self, x, return_feature=False, mode='cls'):
        ...
        if mode == 'rotate':
            return self.rotate_output(f.squeeze())
        elif mode == 'jigsaw':
            return self.jigsaw_output(f.squeeze())
